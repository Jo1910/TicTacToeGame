...
function placeXOrO(squareNumber) {
  ...
  function checkWinConditions() {
    ...
    // O 0, 1, 2 condition
    else if (arrayIncludes("1O,", "2O", "3O")) {
      drawWinLine(50, 100, 558, 100);
    }
    // O 3, 4, 5 condition
    ...
    // It is used to check for any win condition.
    function arrayIncludes(squareA, squareB, squareC) {
      // The next 3 variables will be used to check for 3 in a row.
      const a = selectedSquares.includes(squareA);
      const b = selectedSquares.includes(squareB);
      const c = selectedSquares.includes(squareC);
      // If the 3 variables we pass are all included in our array true is returned and our else if condition executes the draWinLine function
  ...
  // This function takes a string parameter of the path you set earlier for placement sound ('./media/click.wav').
...
function resetGame() {
  // This for loop iterates the game through each HTML square element
  for (let i = 0; i < 9; i++) {
